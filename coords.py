""" Coordinates file. 

Coordinates are relative to in-game top-left corner. 
"""

# some user constants
FAST_SLEEP = 0.15
MEDIUM_SLEEP = 0.3

GAME_WIDTH = 960
GAME_HEIGHT = 600

BASIC_TRAINING = 235, 45
BASIC_TRAINING_ADD = 842, 166
ATK1 = 912, 164
ATK2 = 914, 198
ATK3 = 911, 233
ATK4 = 911, 263
ATK5 = 912, 297
ATK6 = 913, 327
DEF1 = 912, 408
DEF2 = 914, 444
DEF3 = 913, 475
DEF4 = 918, 507
DEF5 = 913, 540
DEF6 = 913, 570

ENERGY_CUSTOM_AMOUNT_CAP = 674, 44
ENERGY_CUSTOM_AMOUNT_HALF = 704, 46
ENERGY_CUSTOM_AMOUNT_QUARTER = 734, 43
ENERGY_CUSTOM_AMOUNT_HALF_IDLE = 868, 46
ENERGY_CUSTOM_AMOUNT_QUARTER_IDLE = 897, 43
MAGIC_CUSTOM_AMOUNT_CAP = 672, 69
MAGIC_CUSTOM_AMOUNT_HALF = 704, 67
MAGIC_CUSTOM_AMOUNT_QUARTER = 734, 67
MAGIC_CUSTOM_AMOUNT_HALF_IDLE = 868, 67
MAGIC_CUSTOM_AMOUNT_QUARTER_IDLE = 897, 67

INPUT_VALUE_TEXT = 420, 45


FIGHT_BOSS = 230, 73
NUKE = 627, 177
STOP = 621, 232
FIGHT = 629, 288

MONEY_PIT = 239, 102
FEED_ME = 420, 190
FEED_YEAH = 430, 344

ADVENTURE = 231, 128

ABILITY_OFFSET_X = 104
ABILITY_OFFSET_Y = 38
ABILITY_1 = 463, 119  # Regular attack
# Ability 1 pixel is to take a screenshot only of the pixel we need to check
# ABILITY_1_REGION = (463, 119, 2, 2) # PYAUTOGUI REGION

ABILITY_1_REGION = (463, 119, 465, 121)  # d3dshot REGION
ABILITY_ROW_1_READY_COLOR = 248, 155, 155
ABILITY_ROW_2_READY_COLOR = 102, 135, 163
ABILITY_ROW_3_READY_COLOR = 195, 148, 148

CHARGE_COOLDOWN = 751, 160
PARRY_COOLDOWN = 644, 131
BEAST_COOLDOWN = 576, 206


IS_IDLE = 414, 112
IS_IDLE_COLOR = (255, 235, 4)
BEAST_MODE = 580, 200
BEAST_MODE_ON = 580, 216
BEAST_MODE_COLOR = (255, 235, 4)
ADVANCE_ZONE = 942, 235
GO_BACK_ZONE = 732, 237
# SNIPE COORDS
MY_HEALTH_BAR_BORDER = 320, 439
MY_HEALTH_BAR_THRESHOLD = 482, 440
MY_HEALTH_BAR_FULL = 513, 439  # 100% of player's health bar
ENEMY_HEALTH_BAR = 770, 440
ENEMY_HEALTH_BAR_WHITE = 250, 250, 250  # KONGREGATE DISCREPANCIES
HEALTH_BAR_RED = 235, 52, 52
ENEMY_HEALTH_BAR_RED = 230, 51, 51
CROWN_LOCATION = 737, 307
CROWN_COLOR = 247, 239, 41
ENEMY_HEALTH_BAR_BORDER = 735, 439
ENEMY_TEXT = 753, 353
ENEMY_BACKGROUND = (102, 135, 163)

ITOPOD_ENTER = 371, 252
ITOPOD_OPTIMAL = 701, 231
ITOPOD_MAX = 706, 268
ITOPOD_START_INPUT = 629, 224
ITOPOD_END_INPUT = 625, 261
ITOPOD_ENTER_CONFIRMATION = 614, 321
ITOPOD_PERKS = 452, 252
ITOPOD_CLICK_TOOLTIP = 511, 49

INVENTORY = 236, 157
INV_DIFF = 50
ACC1 = 481, 87
ACC2 = 481, 139
ACC3 = 481, 189
HEAD = 529, 82
CHEST = 529, 138
LEGS = 537, 188
BOOTS = 532, 241
WEAPON = 585, 138
CUBE = 625, 138
SLOT1 = 356, 354

LOADOUT1 = 330, 285
LOADOUT2 = 360, 285
LOADOUT3 = 390, 285


AUGMENTATION = 236, 187
AUG_DIFF = 65  # on y
AUG1 = 542, 288
AUG1_UPGRADE = 542, 318
AUG6 = 538, 475
AUG6_UPGRADE = 538, 500
AUG7 = 538, 538
AUG7_UPGRADE = 538, 565


# plus buttons
ADV_TRAINING = 229, 212
ADV_TOUGHNESS = 892, 255
ADV_POWER = 892, 293
ADV_BLOCK = 892, 333
ADV_WANDOOS_ENERGY = 892, 374
ADV_WANDOOS_MAGIC = 892, 413


TIME_MACHINE = 230, 237
TM_ADD_ENERGY = 530, 259
TM_ADD_MAGIC = 529, 357

BLOOD_MAGIC = 226, 264
BM1_ADD = 494, 253
BM1_CAP = 567, 251
BM_DIFF = 35  # y
BM_CAP_ALL = 823, 138

WANDOOS = 231, 292
WANDOOS_ENERGY_ADD = 555, 275
WANDOOS_MAGIC_ADD = 555, 375
WANDOOS_ENERGY_CAP = 626, 277
WANDOOS_MAGIC_CAP = 631, 371

NGU = 231, 318

NGU_TO_MAGIC_BUTTON = 378, 133

NGUS_ENERGY = {
    'AUGMENTS':        (516, 265),
    'WANDOOS':         (516, 300),
    'RESPAWN':         (516, 335),
    'GOLD':            (516, 370),
    'ADVENTURE_ALPHA': (516, 405),
    'POWER_ALPHA':     (516, 440),
    'DROP_CHANCE':     (516, 475),
    'MAGIC_NGU':       (516, 510),
    'PP':              (516, 545),
}

NGUS_MAGIC = {
    'YGGDRASIL':       (516, 265),
    'EXP':             (516, 300),
    'POWER_BETA':      (516, 335),
    'NUMBER':          (516, 370),
    'TIME_MACHINE':    (516, 405),
    'ENERGY_NGU':      (516, 440),
    'ADVENTURE_BETA':  (516, 475),
}

NGUS_ENERGY_CAP = {
    'AUGMENTS':        (590, 265),
    'WANDOOS':         (590, 300),
    'RESPAWN':         (590, 335),
    'GOLD':            (590, 370),
    'ADVENTURE_ALPHA': (590, 405),
    'POWER_ALPHA':     (590, 440),
    'DROP_CHANCE':     (590, 475),
    'MAGIC_NGU':       (590, 510),
    'PP':              (590, 545),
}

NGUS_MAGIC_CAP = {
    'YGGDRASIL':       (590, 265),
    'EXP':             (590, 300),
    'POWER_BETA':      (590, 335),
    'NUMBER':          (590, 370),
    'TIME_MACHINE':    (590, 405),
    'ENERGY_NGU':      (590, 440),
    'ADVENTURE_BETA':  (590, 475),
}


YGGDRASIL = 235, 347
FRUITS = {
    'GOLD':          (349, 201),
    'POWER_ALPHA':   (558, 201),
    'ADVENTURE':     (769, 200),
    'KNOWLEDGE':     (348, 299),
    'POMEGRANATE':   (557, 298),
    'LUCK':          (772, 295),
    'POWER_BETA':    (348, 394),
    'ARBITRARINESS': (619, 365),
    'NUMBERS':       (770, 393)
}
FRUITS_IS_HARVESTED = {
    'GOLD':          (338, 171),
    'POWER_ALPHA':   (544, 173),
    'ADVENTURE':     (743, 173),
    'KNOWLEDGE':     (338, 269),
    'POMEGRANATE':   (533, 269),
    'LUCK':          (744, 271),
    'POWER_BETA':    (321, 365),
    'ARBITRARINESS': (533, 365),
    'NUMBERS':       (744, 364)
}
HARVEST_ALL_MAX_TIER = 815, 474

DIGGERS = {
    'DROP_CHANCE':   (550, 207),
    'WANDOOS':       (865, 208),
    'STAT':          (550, 396),
    'ADVENTURE':     (865, 396),
    'ENERGY_NGU':    (550, 207),
    'MAGIC_NGU':     (865, 208),
    'ENERGY_BEARD':  (550, 396),
    'MAGIC_BEARD':   (865, 396),
    'PP':            (550, 207),
    'DAYCARE':       (865, 208),
    'BLOOD':         (550, 396),
    'EXP':           (865, 396)
}

DIGGERS_PAGE_1 = 343, 130
DIGGERS_PAGE_2 = 408, 130
DIGGERS_PAGE_3 = 475, 130
GOLD_DIGGERS = 232, 373
CLEAR_ACTIVE = 908, 118
SAVE_ACTIVE = 800, 110
CAP_SAVED = 800, 140

QUESTING_MENU = 235, 425
QUESTING_COMPLETE_QUEST = 712, 185
QUESTING_SKIP_QUEST = 862, 185
QUESTING_SKIP_QUEST_CONFIRMATION = 434, 340
QUESTING_TEXT_REGION = (303, 325, 312, 200)
QUESTING_PROGRESS_REGION = (389, 406, 50, 18)

REBIRTH_MENU = 91, 445
REBIRTH_BUTTON = 547, 542
REBIRTH_CONFIRMATION = 439, 344

CONFIG = 32, 575
AUTOKILL_TITAN_ON = 507, 321
AUTOKILL_TITAN_OFF = 580, 321

# OCR
TITAN_COUNTER_REGION = (260, 151, 254, 90)

# BIG TEXT, DOESNT WORK (??)
ADVENTURE_ZONE_REGION_TEST = (380, 40, 485, 40)
ADVENTURE_ZONE_REGION = (628, 87, 188, 17)
ADVENTURE_HOVER = 598, 67

EXP_MENU = 91, 480
EXP_REGION = (592, 99, 100, 20)
BOSS_NUMBER_REGION = (785, 334, 80, 20)

ITOPOD_PP_REGION = (764, 45, 100, 30)
ITOPOD_TIER_COUNT_REGION = (537, 140, 77, 18)
ITOPOD_TIER_COUNT_REGION_LOWER_TIERS = (745, 125, 40, 18)

BUTTON_LOCKED_COLOR = (150, 167, 181)
TM_LOCKED_IRON_PILL = (123, 75, 147)

AUGS_LOCKED = 182, 191
TM_LOCKED = 182, 237
BM_LOCKED = 182, 271

# challenges
CHALLENGES_MENU = 700, 545
CHALLENGE_OCR_REGION = 310, 109, 415, 56
CHALLENGES_QUIT = 867, 130
CHALLENGES = {
    'BASIC': (387, 205),
    'NO_AUGS': (387, 235),
    '24_HOUR': (387, 265),
    '100_LEVEL': (387, 295),
    'NO_EQUIPMENT': (387, 325),
    'TROLL': (387, 355),
    'NO_REBIRTH': (387, 385),
    'LASER_SWORD': (387, 415),
    'BLIND': (387, 445),
    'NO_NGU': (387, 475),
    'NO_TM': (387, 505),
}
CHALLENGES_YEAH = 436, 340
